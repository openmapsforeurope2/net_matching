...
namespace app
{
    namespace calcul
    {
        ...
        void EdgeCleaningOp::_init()
        {
            ...
            std::string const allEdgeTable = themeParameters->getValue(ALL_EDGE_TABLE).toString();
            ...
            //--
            _fsAllEdge = context->getDataBaseManager().getFeatureStore(allEdgeTable, idName, geomName);
        ...
        ///
        ///
        bool EdgeCleaningOp::_isCl(std::string const& country) const 
        {
        ...
        ///
        ///
        bool EdgeCleaningOp::_isCurrentBorderCl(std::string const& country) const 
        {
			...
			for( graph.faces( fit, fend ) ; fit != fend ; ++fit, ++display )
			{
				...
				if (_isSlimSurface(faceGeom, slimSurfaceWidth)) {
                    ...
                    do{
                        ...
                        size_t targetDegree = _getDegree(graph, graph.target(nextEdge), sEdge2Remove);
                        if (targetDegree > 2) {
        ...
        bool EdgeCleaningOp::_getFacePaths(
            ...
            do {
                ...
                size_t targetDegree = _getDegree(graph, graph.target(currentEdge), sEdge2Remove);

                if (targetDegree > 2 || graphManager.isCp(graph.target(currentEdge)) || currentCountry != nextCountry ) {
            ...
            if (vpCountryEdges.size() > 1 && vpCountryEdges.front().first == vpCountryEdges.back().first) {
                ...
                if ( graph.degree(v) == 2 && !graphManager.isCp(v) ) {
        ...
        ///
        ///
        size_t EdgeCleaningOp::_getDegree(
            GraphType const& graph,
            vertex_descriptor v,
            std::set<edge_descriptor> const& sEdge2Remove
        ) const {
            ...
            for ( size_t i = 0 ; i < vEdges.size() ; ++i ) {
                if( sEdge2Remove.find(vEdges[i].descriptor) != sEdge2Remove.end() )
                    continue;
                ++degree;
            }
            return degree;
        ...
        void EdgeCleaningOp::cleanFaces2ByCountry() const
        {
            ...
            for (size_t i = 0 ; i < _vCountry.size() ; ++i) {
                cleanFaces2(countryCodeName +" = '"+_vCountry[i]+"'");
            }
        }
        ...
        ///
        ///
        void EdgeCleaningOp::cleanFacesAndAntennaByCountry(
            std::string const& sqlFilter,
            bool tagTreatedDangles
        ) const {
            epg::Context* context = epg::ContextS::getInstance();
            epg::params::EpgParameters const& epgParams = context->getEpgParameters();
            std::string const countryCodeName = epgParams.getValue(COUNTRY_CODE).toString();
            ...
            for (size_t i = 0 ; i < _vCountry.size() ; ++i)
            {
                ...
                epg::tools::FilterTools::addAndConditions(filter, countryCodeName +" LIKE '%"+_vCountry[i]+"%'");
        ...
        ///
        ///
        bool EdgeCleaningOp::_cleanFaces2(
            detail::EdgeCleaningGraphManager & graphManager,
            std::string country
        ) const
        {
            ...
            while ( _cleanGraphFaces(graphManager, country) ) {
                bChangeOccured = true;
        ...
        ///
        ///
        bool EdgeCleaningOp::_cleanGraphFaces(
            detail::EdgeCleaningGraphManager & graphManager,
            std::string country
        ) const {
            bool bChangeOccured = false;
            ...
            for( graph.faces( fit, fend ) ; fit != fend ; ++fit, ++display )
			{
				...
				if (
                    ...
                    _isSlimSurface(faceGeom, slimSurfaceWidth, &p1, &p2) // TODO recuperer v1 et v2
                ) {
                    ign::feature::Feature feat;
                    ...
                    // si oui --> continue
                    std::set<std::string> isAllFromCountry = _isAllFromCountry(graphManager, *fit);
                    if(!isAllFromCountry.empty()) {
                        if(isAllFromCountry.size() > 1 || _intersectsCountry(faceGeom, *isAllFromCountry.begin()))
                    ...
                    if (!_getFacePaths(graphManager, *fit, vpCountryEdges, sEdge2Remove))
                        continue;
                    
                    if (vpCountryEdges.size() == 1 && !_isCl(vpCountryEdges.begin()->first)) {
                        ign::feature::Feature feat;
                        feat.setGeometry(faceGeom);
                    ...
                    if (bUse1stMethode) {
                        ...
                        hasConnection1 = _hasConnection(graph, branch1, sEdge2Remove);
                        hasConnection2 = _hasConnection(graph, branch2, sEdge2Remove);    
                    }
                    ...
                    // - c'est la seule des 2 à n'avoir pas de connexion
                    // - elle a le moins bon ration et les 2 branches n'ont pas de connexion
        ...
        ///
        ///
        std::set<std::string> EdgeCleaningOp::_isAllFromCountry(
            detail::EdgeCleaningGraphManager & graphManager,
            face_descriptor fd
        ) const {
            GraphType const& graph = graphManager.getGraph();
        ...
        ///
        ///
        bool EdgeCleaningOp::_hasConnection (
            GraphType const& graph,
            ...
            for ( size_t i = 0; i < branch.size() ; ++i)
            {
                for ( std::list<oriented_edge_descriptor>::const_iterator lit = branch[i].second.begin() ; lit != branch[i].second.end() ; ++lit )
                {
                    ...
                    size_t targetDegree = _getDegree(graph, graph.target(*lit), sEdge2Remove);
                    if ( targetDegree > 2 ||  graph[graph.target(*lit)].isCp ) {
                        return true;
                    }
                }
            }
        ...
        void EdgeCleaningOp::_cleanFacesAntennas(
            ...
            for (sit = sVertices.begin() ; sit != sVertices.end() ; ++sit)
            {
                ...
                if( graphManager.isCp(*sit) ) continue;
                
                oriented_edge_descriptor oe = _getIncidentEdge(graph, *sit);

                //dans le cas d'un traitement pays par pays, on vérifie, si l'antenne est une CL,
                //qu'elle n'est pas connectée au réseau de l'autre pays avant de la supprimer
                bool connected2OtherCountry = _isCurrentBorderCl(graphManager.getCountry(oe.descriptor));
                if( country != ""
                    && connected2OtherCountry
                    && _isConnected2OtherCountry( graph.getGeometry(*sit), country )
                ) continue;

                //connection a un pays tiers ?
                if (_hasConnectedThirdCountry(graph.getGeometry(*sit), graphManager.getCountry(oe.descriptor)))
                    continue;

                std::pair<bool, std::list<oriented_edge_descriptor>> pAntenna = _getAntenna(graphManager, *sit, sTreatedFeatures, true/*is planar*/, true /*withCl --> à confirmer*/);

                //DEBUG
                ign::feature::Feature feat;
                feat.setGeometry(graph.getGeometry(pAntenna.second.begin()->descriptor));
                _shapeLogger->writeFeature("removed_face_antenna", feat);
        ...
        ///
        ///
        bool EdgeCleaningOp::_hasConnectedThirdCountry(
            ign::geometry::Point const& vertexGeom,
            std::string const& countryCode
        ) const {
            std::set<std::string> sThirdCountry = _getThirdCountry(countryCode);
            for (std::set<std::string>::const_iterator sit = sThirdCountry.begin() ; sit != sThirdCountry.end() ; ++sit)
                if( _isConnected2ThirdCountry( vertexGeom, *sit ) ) 
                    return true;
        ...
        ///
        ///
        std::set<std::string> EdgeCleaningOp::_getThirdCountry(std::string const& country) const
        {
            std::set<std::string> sThirdCountry;
            ...
            for (std::vector<std::string>::iterator vit = vCountry.begin() ; vit != vCountry.end() ; ++vit)
                if(_isThirdCountry(*vit))
                    sThirdCountry.insert(*vit);

            return sThirdCountry;
        }

        ///
        ///
        ///
        bool EdgeCleaningOp::_isThirdCountry(std::string const& singleCountry) const
        {
            return _borderCode.find(singleCountry) == std::string::npos;
        }

        ///
        ///
        ///
        std::string EdgeCleaningOp::_getOtherCountry(
            std::string const& country
        ) const {
        ...
        ///
        ///
        bool EdgeCleaningOp::_isConnected2OtherCountry(
            ...
            //--
			ign::feature::FeatureFilter filterEdge(countryCodeName + " LIKE '%" + otherCountry + "%'");
			epg::tools::FilterTools::addAndConditions(filterEdge, countryCodeName + " NOT LIKE '%" + country + "%'");

            //--
            std::string ptSql = "ST_GeomFromText('" + pt.toString() + "', 3035)";
            std::ostringstream oss;
            oss << geomName << " && ST_Expand(" << ptSql << ", 0.01)";
            oss << " AND (";
            oss << "ST_DWithin(ST_StartPoint(" << geomName << "), " << ptSql << ", 0.01)";
            oss << " OR ";
            oss << "ST_DWithin(ST_EndPoint(" << geomName << "), " << ptSql << ", 0.01)";
            oss << ")";

			epg::tools::FilterTools::addAndConditions(filterEdge, oss.str());
        ...
        ///
        ///
        bool EdgeCleaningOp::_isConnected2ThirdCountry(
            ign::geometry::Point const& pt,
            ...
            //--
            ign::feature::FeatureFilter filterEdge(countryCodeName + " LIKE '%" + thirdCountryCode + "%'");
			epg::tools::FilterTools::addAndConditions(filterEdge, countryCodeName + " NOT LIKE '%" + _vCountry.front() + "%'");
			epg::tools::FilterTools::addAndConditions(filterEdge, countryCodeName + " NOT LIKE '%" + _vCountry.back() + "%'");
            ...
            //--
            size_t numFeatures = ome2::feature::sql::NotDestroyedTools::NumFeatures(*_fsAllEdge, filterEdge);
        ...
        ///
        ///
        app::calcul::detail::EdgeCleaningGraphManager::oriented_edge_descriptor EdgeCleaningOp::_getIncidentEdge(
            GraphType const& graph,
        ...
        std::pair<bool, std::list<app::calcul::detail::EdgeCleaningGraphManager::oriented_edge_descriptor>> EdgeCleaningOp::_getAntenna(
            ...
            oriented_edge_descriptor nextEdge = _getIncidentEdge(graph, v); // si nextEdge n'est pas une CL ?
            bool previousIsCl = graphManager.isCl(nextEdge.descriptor);
            std::string previousCountry = graphManager.getCountry(nextEdge.descriptor);

            if ( !withCl && previousIsCl ) {
            ...
            if ( previousIsCl && _hasConnectedThirdCountry(graph.getGeometry(v), previousCountry) )
                return std::make_pair(isConnected2CF, lAntennaEdges);
            ...
            while( true )
            {
                ...
                if( !withCl && previousCountry != currentCountry ) {
                    isConnected2CF = true;
                    break;
                }
                if( withCl ) {
                    if( currentIsCl )
                        if( _hasConnectedThirdCountry(graph.getGeometry(graph.source(nextEdge)), currentCountry) ) {
                            isConnected2CF = true;
                            break;
                        }
                    else if ( previousIsCl )
                        if( _hasConnectedThirdCountry(graph.getGeometry(graph.source(nextEdge)), previousCountry) ) {
                            isConnected2CF = true;
                            break;
                        }
                    if( !_haveCommonCountry( currentCountry, previousCountry ) ) {
                        isConnected2CF = true;
                        break;
                    }
                }
                ...
                bool targetIsCp = graphManager.isCp(vTarget);
                if( targetIsCp )
                    isConnected2CF = true;
        ...
        ///
        ///
        bool EdgeCleaningOp::_haveCommonCountry(
            std::string const& country1,
            std::string const& country2
        ) const {
            std::set<std::string> s1;
            epg::tools::StringTools::Split(country1, "#", s1);

            std::set<std::string> s2;
            epg::tools::StringTools::Split(country2, "#", s2);
        ...
        ///
        ///
        bool EdgeCleaningOp::_vertexIsConnected2Cl(
            detail::EdgeCleaningGraphManager const& graphManager,
            vertex_descriptor v
        ) const {
            std::vector< oriented_edge_descriptor > vEdges;
        ...
        ///
        ///
        void EdgeCleaningOp::_initVertexIsCp(detail::EdgeCleaningGraphManager & graphManager) const
        {
            ...
            while (itCp->hasNext())
            {
                ...
                std::set< vertex_descriptor > sVertices;
		        graph.getVertices( cpGeom.getEnvelope().expandBy( threshold ), sVertices );

                double dMax = threshold;
                vertex_descriptor vMax;
                bool found = false;
                for (std::set< vertex_descriptor >::const_iterator sit = sVertices.begin() ; sit != sVertices.end() ; ++sit) {
                    double distance = graph.getGeometry(*sit).distance(cpGeom);
                    if( distance < dMax ) {
                        dMax = distance;
                        vMax = *sit;
                        found = true;
        ...
        void EdgeCleaningOp::cleanPathsOutOfCountry() const
        {
            ...
            for (mit = _mCountryGeomPtr.begin() ; mit != _mCountryGeomPtr.end() ; ++mit)
            {
                ...
                {
                    if (_vertexIsConnected2Cl(graphManager, *vit) || graphManager.isCp(*vit))
        ...
        bool EdgeCleaningOp::_cleanGraphAntennas(
            ...
            {
                //DEBUG
                // _logger->log(epg::log::DEBUG, graph.getGeometry(*vit).toString());
                // if( graph.getGeometry(*vit).distance(ign::geometry::Point(4069329.46, 2961417.09)) < 1 ) {
                //     bool test = true;
                // }

                if( graph.degree( *vit ) != 1 ) continue;
                if( graphManager.isCp(*vit) ) continue;

                std::string edgeFeatId = graph.origins(_getIncidentEdge(graph, *vit).descriptor)[0];
            ...
            {
                //DEBUG
                // if( graph.getGeometry(mit->first).distance(ign::geometry::Point(4057842.65, 2935738.17)) < 1){
                //     bool test =true;
                // }
                ...
                // s'il reste une antenne et qu'il y en avait plusieur initialement
                // on supprime cette antenne des antennes visitées car elle peut donner
                // naissance à une nouvelle antenne plus longue
                // Permet egalement de gérer arc isolé 
                if ( mLengthAntenna.size() == 1 && !hasOnlyOneAntenna ) {
